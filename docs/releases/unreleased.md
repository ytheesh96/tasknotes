...
- Prevented Hermes sync from repeatedly rewriting unchanged Bases-backed task and activity notes by scoping activity note filenames to their source task, preserving stored activity sync timestamps for unchanged curated activity, and suppressing no-op mirror writes, reducing kanban view flicker during background reconciliation.
