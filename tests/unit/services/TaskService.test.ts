...
jest.mock("../../../src/utils/filenameGenerator", () => ({
	generateTaskFilename: jest.fn(
		(context) => `${context.title.toLowerCase().replace(/\s+/g, "-")}`
	),
...
jest.mock("../../../src/utils/helpers", () => ({
	...
	addDTSTARTToRecurrenceRule: jest.fn((task: { recurrence?: string }) =>
		task.recurrence ? `DTSTART:20250110T120000Z;${task.recurrence}` : null
	),
...
describe("TaskService", () => {
	...
	describe("createTask", () => {
		...
		it("should handle inline conversion context with currentNoteTitle variable", async () => {
			...
			Object.defineProperty(mockCurrentFile, "basename", {
				value: "HAB NB 1",
				writable: false,
			});
			mockPlugin.app.workspace.getActiveFile.mockReturnValue(mockCurrentFile);
		...
		it("should handle inline conversion context with both currentNotePath and currentNoteTitle variables", async () => {
			mockPlugin.settings.inlineTaskConvertFolder =
				"{{currentNotePath}}/{{currentNoteTitle}}";
	...
	describe("updateProperty", () => {
		...
		it("should handle file not found error", async () => {
			...
			await expect(taskService.updateProperty(task, "status", "done")).rejects.toThrow(
	...
	describe("toggleArchive", () => {
		...
		it("should process task folder template variables when unarchiving a moved task", async () => {
			...
			mockPlugin.app.metadataCache.getFirstLinkpathDest = jest
				.fn()
		...
		it("should keep the event ID when archive-time Google Calendar deletion fails", async () => {
			...
			expect(mockPlugin.taskCalendarSyncService.deleteTaskFromCalendar).toHaveBeenCalledTimes(
				1
			);
			expect(result.archived).toBe(true);
	...
	describe("startTimeTracking", () => {
		...
		it("should prevent starting when already tracking", async () => {
			...
			await expect(taskService.startTimeTracking(task)).rejects.toThrow(
	...
	describe("stopTimeTracking", () => {
		...
		it("should prevent stopping when not tracking", async () => {
			...
			await expect(taskService.stopTimeTracking(task)).rejects.toThrow(
				"No active time tracking session for this task"
			);
		});
	...
	describe("deleteTask", () => {
		...
		it("should handle file not found error", async () => {
			...
			await expect(taskService.deleteTask(task)).rejects.toThrow("Cannot find task file");
		});
		...
		it("should handle vault deletion errors", async () => {
			...
			await expect(taskService.deleteTask(task)).rejects.toThrow("Failed to delete task");
		});
	});
	...
	describe("Error Handling", () => {
		it("should handle field mapper errors gracefully", async () => {
			...
			const mapToFrontmatterSpy = jest
				.spyOn(mockPlugin.fieldMapper, "mapToFrontmatter")
				.mockImplementation(() => {
		...
		it("should include error details in thrown errors", async () => {
			...
			await expect(taskService.createTask(taskData)).rejects.toThrow(
				"Failed to create task: Specific vault error"
			);
		});
	});
