...
describe("taskArchivePlanning", () => {
	...
	it("uses hermesArchived instead of the archive tag for Hermes-managed tasks", () => {
		const plan = buildTaskArchiveState(
			createTask({
				archived: false,
				tags: ["task"],
	...
	it("lets hermesArchived false override an archive tag when unarchiving state is read", () => {
		const plan = buildTaskArchiveState(
			createTask({
				archived: true,
				status: "done",
